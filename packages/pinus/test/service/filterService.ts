...
let mockFilter1 = {
    before: function (record: any, msg: any, session: FrontendOrBackendSession & { beforeCount1: number }, cb: Function) {
        session.beforeCount1++;
        cb();
    },

    after: function (err: Error, record: any, msg: any, session: FrontendOrBackendSession & { afterCount1: number }, resp: any, cb: Function) {
        session.afterCount1++;
        cb();
    }
};
...
let mockFilter2 = {
    before: function (record: any, msg: any, session: FrontendOrBackendSession & { beforeCount2: number }, cb: Function) {
        session.beforeCount2++;
        cb();
    },

    after: function (err: Error, record: any, msg: any, session: FrontendOrBackendSession & { afterCount2: number }, resp: any, cb: Function) {
        session.afterCount2++;
        cb();
    }
};
...
describe('filter service test', function () {
    describe('#filter', function () {
        ...
        it('should be ok if filter is a function', function (done: MochaDone) {
            ...
            service.before(function (record: any, msg: any, session: FrontendOrBackendSession & { beforeCount: number }, cb: Function) {
                session.beforeCount++;
                cb();
            });
            service.after(function (err: Error, record: any, msg: any, session: FrontendOrBackendSession & { afterCount: number }, resp: any, cb: Function) {
                session.afterCount++;
                cb();
            });
            service.beforeFilter(null, {}, session, function () {
        ...
        it('should pass the err and resp parameters to callback and ignore the filters behind if them specified in before filter', function (done: MochaDone) {
            ...
            let respFilter = {
                before: function (record: any, msg: any, session: FrontendOrBackendSession, cb: Function) {
                    cb(error, response);
                }
            };
